...
void seBackEventHandle(SPWindow** window,SDL_Event *event) {
	...
	if(simpleWindow->type == CH_WINDOW_MODE || simpleWindow->type == CH_WINDOW_LOAD){
		...
		*window = createSimpleStartWindow(); // create window
		if (*window == NULL ) {
		...
		destroyWindow(*window);
		*window = createSimpleModeWindow(); // create window
		if (*window == NULL ) {
		...
		destroyWindow(*window);
		*window = createSimpleDifficultyWindow(); // create window
		if (*window == NULL ) {
			printf("ERROR: unable to create difficulty window: %s\n", SDL_GetError());
			seMessageTerminate();
		}
...
void seNextEventHandle(SPWindow** window,SDL_Event *event) {
	...
	if(simpleWindow->type == CH_WINDOW_MODE){
		...
		*window = createSimpleDifficultyWindow();
		...
		pressDifficultyChange(*window,gameDifficulty); //parse the difficulty
		SDL_WaitEvent(event);
	}else if(simpleWindow->type == CH_WINDOW_DIFFICULTY){
		destroyWindow(*window);
		*window = createSimpleColorWindow();
		...
		pressColorChange(*window,userColor); //parse the color
		SDL_WaitEvent(event);
	}
}
...
CH_GAME_MESSAGE seLoadWindowEventHandle(SPWindow** window,SDL_Event *event, CHGame** src){
	...
	if(simpleWindow->type == CH_WINDOW_START){
		destroyWindow(*window);
		*window = createLoadWindow(); // create window
		if(*window == NULL){
...
CHGame *seGuiActiveHandle(){
	...
	window = createSimpleStartWindow(); // create start window
	if (window == NULL ) {
		printf("ERROR: unable to create start window: %s\n", SDL_GetError());
		seMessageTerminate();
		return NULL;
	...
	while (true) {
		...
		if(event.type == SDL_QUIT){
			...
			SDL_Quit();
			return NULL;
		}else if(event.type == SDL_USEREVENT){ // handle event by type
...
CHGame* startSettingsMode(bool isGuiMode) {
	...
	if (isGuiMode) {
		...
		while (true) {
			fgets(strCommand, MAX_LINE_SIZE, stdin); // read the game settings from the user
	...
	src = chGameCreate(gameMode, userColor, gameDifficulty, currentTurn); // create the game
	return src;
}
